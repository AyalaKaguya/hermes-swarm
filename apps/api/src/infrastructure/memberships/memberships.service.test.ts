...
function createState() {
  const users = [{ id: "user-a", displayName: "User A", email: "a@example.com", tenantId: "tenant-a" }];
  const organizations = [
  ...
  const repositories = new Map<any, any>([
    ...
    [User, { findOne: async ({ where }: any) => users.find((item) => item.id === where.id && item.tenantId === where.tenantId) ?? null }],
