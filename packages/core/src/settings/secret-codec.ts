...
export function decryptSettingSecret(
  ...
  if (!secret) {
    throw new Error(`Encrypted setting secret uses unknown key: ${keyId}`);
  }
...
export function decryptEncryptedSettingSecret(
  value: string,
  keyring: SettingSecretKeyring,
) {
  if (!value.startsWith(`${ENCRYPTED_SETTING_V2_PREFIX}:`)) {
    throw new Error("Encrypted setting secret must use the current envelope format");
  }
  return decryptSettingSecret(value, keyring);
}
