import type {
  PageAccessDefinition,
  PermissionScope,
} from "./types.js";
import { matchRoutePattern } from "./route-pattern.js";
import { getPageAccessPermissionId } from "./permission-key.js";

export const PAGE_ACCESS_DEFINITIONS = [
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问组织工单和会话页面。",
    href: "/tickets",
    icon: "file",
    key: "tickets",
    label: "工单",
    order: 10,
    routePatterns: ["/tickets", "/tickets/:ticketId"],
    scope: "organization",
    section: "infrastructure",
    sectionLabel: "基础设施",
  }),
  definePageAccess({
    defaultRoles: ["tenant-owner", "tenant-admin", "tenant-member"],
    description: "允许访问当前租户的支持工单和租户内会话页面。",
    href: "/tickets",
    icon: "file",
    key: "tickets.tenant",
    label: "租户工单",
    order: 10,
    routePatterns: ["/tickets", "/tickets/:ticketId"],
    scope: "tenant",
    section: "infrastructure",
    sectionLabel: "基础设施",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问个人账号资料与密码设置页面。",
    href: "/settings/account",
    icon: "user",
    key: "settings.account",
    label: "账号",
    order: 10,
    routePatterns: ["/settings", "/settings/account"],
    scope: "own",
    section: "personal",
    sectionLabel: "个人",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问当前账号的登录设备与会话管理页面。",
    href: "/settings/sessions",
    icon: "system",
    key: "settings.sessions",
    label: "登录设备",
    order: 20,
    routePatterns: ["/settings/sessions"],
    scope: "own",
    section: "personal",
    sectionLabel: "个人",
  }),
  definePageAccess({
    defaultRoles: ["tenant-owner", "tenant-admin", "tenant-member"],
    description: "允许访问当前租户账号可创建的租户、组织和部门集成 Token。",
    href: "/settings/integrations",
    icon: "plug",
    key: "settings.integrations",
    label: "集成",
    order: 30,
    routePatterns: ["/settings/integrations"],
    scope: "tenant",
    section: "personal",
    sectionLabel: "个人",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin"],
    description: "允许访问当前组织的集成 Token 管理页面。",
    href: "/settings/organization-integrations",
    icon: "plug",
    key: "settings.organization-integrations",
    label: "集成管理",
    order: 80,
    routePatterns: ["/settings/organization-integrations"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问当前组织的基础资料页面。",
    href: "/settings/organization",
    icon: "building",
    key: "settings.organization",
    label: "常规",
    order: 10,
    routePatterns: [
      "/settings/organization",
      "/settings/organization-controls",
    ],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin"],
    description: "允许访问组织自定义 SMTP 配置页面。",
    href: "/settings/custom-smtp",
    icon: "mail",
    key: "settings.custom-smtp",
    label: "自定义邮件",
    order: 20,
    routePatterns: ["/settings/custom-smtp"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问组织邮件模板页面。",
    href: "/settings/email-templates",
    icon: "file",
    key: "settings.email-templates",
    label: "邮件模板",
    order: 30,
    routePatterns: ["/settings/email-templates"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问组织通知配置页面。",
    href: "/settings/notification-destinations",
    icon: "bell",
    key: "settings.notification-destinations",
    label: "通知",
    order: 40,
    routePatterns: ["/settings/notification-destinations"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin"],
    description: "允许访问组织功能开关页面。",
    href: "/settings/features",
    icon: "grid",
    key: "settings.features",
    label: "功能",
    order: 50,
    routePatterns: ["/settings/features"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问组织用户组页面。",
    href: "/settings/groups",
    icon: "users",
    key: "settings.groups",
    label: "用户组",
    order: 60,
    routePatterns: ["/settings/groups"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin", "member", "viewer"],
    description: "允许访问组织角色和权限页面。",
    href: "/settings/roles",
    icon: "shield",
    key: "settings.roles",
    label: "角色和权限",
    order: 70,
    routePatterns: ["/settings/roles"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["platform-admin"],
    description: "允许访问平台租户申请审批和租户开通页面。",
    href: "/platform/tenants",
    icon: "building",
    key: "platform.tenants",
    label: "租户申请",
    order: 5,
    routePatterns: ["/platform", "/platform/tenants"],
    scope: "platform",
    section: "platform",
    sectionLabel: "平台",
  }),
  definePageAccess({
    defaultRoles: ["platform-admin"],
    description: "允许访问平台基础设施配置页面。",
    href: "/platform/settings",
    icon: "server",
    key: "settings.platform",
    label: "平台基础设施",
    order: 10,
    routePatterns: ["/platform/settings", "/settings/platform"],
    scope: "platform",
    section: "platform",
    sectionLabel: "平台",
  }),
  definePageAccess({
    defaultRoles: ["tenant-owner", "tenant-admin", "tenant-member"],
    description: "允许访问当前租户资料、组织目录和租户治理入口。",
    href: "/settings/tenant",
    icon: "layers",
    key: "settings.tenant",
    label: "租户控制台",
    order: 5,
    routePatterns: ["/settings/tenant"],
    scope: "tenant",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["owner", "admin"],
    description: "允许访问当前组织的部门树、部门成员和调度关系。",
    href: "/settings/organization/departments",
    icon: "layers",
    key: "settings.organization.departments",
    label: "部门",
    order: 15,
    routePatterns: ["/settings/organization/departments"],
    scope: "organization",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["tenant-owner", "tenant-admin", "tenant-member"],
    description: "允许访问当前租户的组织目录与组织管理入口。",
    href: "/settings/organizations",
    icon: "building",
    key: "settings.organizations",
    label: "组织目录",
    order: 20,
    routePatterns: ["/settings/organizations", "/settings/organizations/:organizationId"],
    scope: "tenant",
    section: "organization",
    sectionLabel: "组织",
  }),
  definePageAccess({
    defaultRoles: ["platform-admin"],
    description: "允许访问平台邮件模板页面。",
    href: "/platform/email-templates",
    icon: "mail",
    key: "settings.platform-email-templates",
    label: "平台邮件模板",
    order: 30,
    routePatterns: [
      "/platform/email-templates",
      "/settings/platform-email-templates",
    ],
    scope: "platform",
    section: "platform",
    sectionLabel: "平台",
  }),
] as const satisfies PageAccessDefinition[];

export function getPageAccessDefinition(pageKey: string) {
  return (
    PAGE_ACCESS_DEFINITIONS.find((definition) => definition.key === pageKey) ??
    null
  );
}

export function findPageAccessDefinitionByPath(pathname: string) {
  return findPageAccessDefinitionsByPath(pathname)[0] ?? null;
}

export function findPageAccessDefinitionsByPath(pathname: string) {
  return (
    PAGE_ACCESS_DEFINITIONS.filter((definition) =>
      definition.routePatterns.some((pattern) => matchRoutePattern(pattern, pathname)),
    )
  );
}

function definePageAccess(
  definition: Omit<PageAccessDefinition, "permission">,
): PageAccessDefinition {
  return {
    ...definition,
    permission: getPageAccessPermissionId(definition.key, definition.scope),
  };
}
