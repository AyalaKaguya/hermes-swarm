...
- 平台连接用于跨工作空间治理，工作空间连接必须使用 `NOBYPASSRLS` 数据库角色。
- 每个工作空间事务设置 `app.workspace_id`；缺少或无效上下文时失败关闭。
- 所有工作空间业务表使用非空 `workspace_id`、复合外键、RLS policy 和 `FORCE ROW LEVEL SECURITY`。
...
- 跨 Workspace 的读取、写入、邀请、角色和工单访问同时被服务层与 RLS 拒绝。
