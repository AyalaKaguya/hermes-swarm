...
- Platform、Tenant、Organization 三层主体和资源边界明确；
- 平台账号与租户账号使用独立身份面；
- 租户请求通过服务端会话取得 Tenant ID，不接受浏览器指定 Tenant Scope；
- Tenant-owned 表启用 PostgreSQL RLS，并使用不能绕过 RLS 的数据库角色；
...
6. Tenant/Organization Owner 连续性约束不完整；
...
- Platform、Tenant、Organization 都必须至少保留一个有效 Owner/Admin。
...
- 不改变 `Platform → Tenant → Organization` 资源模型。
- 不引入跨 Tenant 的统一管理员账号。
...
| Owner 角色 | Platform Admin、Tenant Owner、Organization Owner 为受保护系统角色 |
...
flowchart LR
  ...
  Service --> RLS["PostgreSQL RLS<br/>Tenant + Organization"]
...
2. `tenant-owner` 只能由当前 `tenant-owner` 授予；
...
- `UsersService.create`
- `UsersService.replaceTenantRole`
...
- Tenant Admin 给自己或他人设置 Tenant Owner 返回 `403`；
...
- 禁用 Tenant Owner；
- 删除 Tenant Owner；
- 删除或禁用 Tenant 本身导致 Owner 不可用的管理动作。
...
任何事务提交后必须至少剩余一个有效 Tenant Owner。
...
- Platform Auth；
- Platform/Tenant/Organization Permission Catalog；
- Organization Roles；
- 后续新增 Controller。
...
| 接口 | 维度 | 初始限制 |
| --- | --- | --- |
| Tenant/Platform Login | IP | 30 次/5 分钟 |
| Tenant/Platform Login | Workspace + 邮箱 Hash | 5 次/5 分钟 |
| Password Reset Request | IP | 10 次/15 分钟 |
| Password Reset Request | Workspace + 邮箱 Hash | 3 次/15 分钟 |
| Refresh | Session + IP | 60 次/分钟 |
| Tenant Context Discovery | IP | 60 次/分钟 |
...
平台 Layout 使用与 Tenant Settings 相同的页面权限解析：
...
AND (
  current_setting('app.scope_level', true) = 'tenant'
  OR (
...
- Tenant Scope 可以在授权后读取本 Tenant 的多个 Organization；
- Organization Scope 只能访问一个精确 Organization；
- 缺少或非法 `scope_level/organization_id` 时返回零行或拒绝写入；
- Background Job 必须显式设置 Tenant 和 Organization Scope；
...
- API E2E 7/7 通过，包括真实 PostgreSQL Tenant/Organization RLS；
...
| API E2E | 旧 Access/Refresh Session 失效、Admin 自我晋升拒绝、并发最后 Owner、跨 Tenant/Organization RLS |
...
- Organization RLS 使用独立迁移，必要时只回滚 Organization Predicate，不关闭 Tenant RLS；
...
- [x] 所有凭据变更立即使全部旧会话失效
- [x] Tenant/Organization Admin 无法晋升 Owner
- [x] 自定义角色不能获得操作者自身没有的权限
- [x] Platform/Tenant/Organization 均至少保留一个有效 Owner/Admin
